...
def _make_schema(
    ...
    return ScoutSchema(
        ...
        content=[item for item in evidence_items if item.evidence_type.startswith("CONTENT_")],
        ...
        diagnostics={
            ...
            "content_items": sum(
                1 for i in evidence_items if i.evidence_type.startswith("CONTENT_")
            ),
...
class TestEmptySchema:
    ...
    def test_empty_schema_all_sections_present(self) -> None:
        schema = _make_schema([])
        result = summarize(schema)
        assert "## Executive Scorecard" in result.text
        assert "## Executive Summary" in result.text
        assert "## Business Snapshot" in result.text
        assert "## Signal Coverage" in result.text
        assert "## Website Overview" in result.text
        assert "## SEO Analysis" in result.text
        assert "## Metadata Analysis" in result.text
        assert "## Technology Stack" in result.text
        assert "## Technology Maturity" in result.text
        assert "## Content Analysis" in result.text
        assert "## Social Presence" in result.text
        assert "## Competitive Signals" in result.text
        assert "## Top Strengths" in result.text
        assert "## Improvement Opportunities" in result.text
        assert "## Executive Recommendations" in result.text
        assert "## Technical Diagnostics" in result.text
        assert "## Evidence Appendix" in result.text
        assert "Generated by AIFME Scout OSS" in result.text

    def test_empty_schema_excludes_conditional_sections(self) -> None:
        schema = _make_schema([])
        result = summarize(schema)
        assert "## Scan Limitations" not in result.text
...
class TestPartialSchema:
    def test_partial_schema_no_technology(self) -> None:
        ...
        result = summarize(schema)
        assert "## Technology Stack" in result.text
        assert "No technology evidence was collected during this scan" in result.text
...
class TestTargetClassification:
    def test_general_classification_for_empty_schema(self) -> None:
        ...
        assert classification == "general website"
    ...
    def test_ecommerce_classification(self) -> None:
        ...
        schema = _make_schema(items)
        classification, _ = _classify_target(schema)
        assert classification == "e-commerce platform"
    ...
    def test_saas_classification(self) -> None:
        ...
        classification, _ = _classify_target(schema)
        assert classification == "saas platform"
    ...
    def test_media_classification(self) -> None:
        items = [
            _make_evidence(
                ...
                value={"type": "video", "src": "/video.mp4"},
            ),
        ]
        schema = _make_schema(items, target_url="https://media.example.com")
        classification, _ = _classify_target(schema)
        assert classification == "media platform"

    def test_agency_classification(self) -> None:
        items = [
            _make_evidence(
                evidence_id="ev-000001",
                evidence_type="CONTENT_HEADING",
                value="We are a creative agency",
            ),
        ]
        schema = _make_schema(items, target_url="https://agency.example.com")
        classification, _ = _classify_target(schema)
        assert classification == "agency website"

    def test_educational_classification(self) -> None:
        items = [
            _make_evidence(
                evidence_id="ev-000001",
                evidence_type="CONTENT_HEADING",
                value="Welcome to our university",
            ),
        ]
        schema = _make_schema(items, target_url="https://university.edu")
        classification, _ = _classify_target(schema)
        assert classification == "educational institution"
    ...
    def test_word_boundary_matching(self) -> None:
        items = [
            _make_evidence(
                evidence_id="ev-000001",
                evidence_type="CONTENT_HEADING",
                value="This is about shops and shopping carts",
            ),
        ]
        schema = _make_schema(items)
        classification, _ = _classify_target(schema)
